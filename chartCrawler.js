...
1. Added twa audit tool
2. Added lactic acid, INR, patient history, ventmode to the labExtractor()
3. Made changes in the urine output puller
4. Changed vital pulling mechanism of labExtraction
5. Broke down the whole chartCrawler to five different functions
6. twa tool cell changed from W to X
7. Added culLink tool, dcdisp tool, added history range extraction in labExtraction tool. 
...
function chartCrawler() {
    ...
    var rootHospFolder = DriveApp.getFolderById("13bX64sIOQaveZSqvBz1_lYV9ia1uICb-"); //"MRNH" folder ID
    var hospitalFolders = rootHospFolder.getFolders();
    var files, oldChartFileId, phFolders, monthFolders, currentMonth, yearFolders, year;
    ...
    while (hospitalFolders.hasNext()) {
      ...
      if (phFolders.getName() == "Old Charts") {
        ...
        while (yearFolders.hasNext) {
          ...
          if (year.getName() == "2019") {
            monthFolders = year.getFolders();
            while (monthFolders.hasNext()) {
              currentMonth = monthFolders.next();
              if(currentMonth.getName() == "February 2019") { //DESTINATION FOLDER TO PARSE//
                ...
                while (files.hasNext()) {
                  ...
                  dcdisp = getdcdisp(oldChartFileId, max);
                  ...
                  var labarray = [];
                  labarray = labExtraction(oldChartFileId);
                  ...
                  //COUNTER TO LIMIT NUMBER OF FILES TO PARSE
                }
              }
            }
          }
  ...
  function getdcdisp (oldChartFileId, max) {
    ss = SpreadsheetApp.openById(oldChartFileId); 
    var sheets = SpreadsheetApp.openById(oldChartFileId).getSheets(); 
    for (cnt = (max); cnt > 0; cnt--) { 
  ...
  function labExtraction(oldChartFileId) {
